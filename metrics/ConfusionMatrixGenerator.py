...
class ConfusionMatrixGenerator:
    """ Generates a Confusion Matrix heatmap graph.
    ...
    def __configcell_text_and_colors(array_df, lin, col, oText, facecolors, posi, fz, fmt, show_null_values=0):
        """
        ...
        if(col == (ccl - 1)) or (lin == (ccl - 1)):
            ...
            per_ok_s = ['{per_ok:{fmt}}%'.format(fmt=fmt, per_ok=per_ok), '100%'][per_ok == 100]
            ...
            # Hide sum cell (last line and last column) that cross over Recall and Precision
            if(col == ccl - 1) and (lin == ccl - 1):
                lis_txt = []
            else:
                lis_txt = [per_ok_s]
            ...
            if(per > 0):
                txt = '{cell_val}\n{per:{fmt}}%'.format(cell_val=cell_val, fmt=fmt, per=per)
            else:
    ...
    def __pretty_plot_confusion_matrix(df_cm, annot=True, cmap="Oranges", fmt='.2f', fz=11,
        ...
        # create "Total" column
        ConfusionMatrixGenerator.__insert_totals(df_cm)

        # this is for print allways in the same window
        fig, ax1 = ConfusionMatrixGenerator.__get_new_fig('Conf matrix default', figsize)
        ...
        for t in ax.collections[0].axes.texts:  # ax.texts:
            ...
            # set text
            txt_res = ConfusionMatrixGenerator.__configcell_text_and_colors(
    ...
    def __plot_confusion_matrix_from_data(y_test, predictions, columns=None, annot=True, cmap="Oranges",
        ...
        ConfusionMatrixGenerator.__pretty_plot_confusion_matrix(df_cm, fz=fz, cmap=cmap, figsize=figsize,
